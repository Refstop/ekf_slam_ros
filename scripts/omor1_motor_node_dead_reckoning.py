...
from ekf_slam_ros.msg import Dead_reckoning
...
class OMOR1MotorNode:
   def __init__(self):
      ...
      # subscriber
      rospy.Subscriber("cmd_vel", Twist, self.cbSubCmdVelTMsg, queue_size=1)   # use "smooth_cmd_vel" for smooth path     # command velocity data subscriber
      ...
      self.dead_reckoning_pub = rospy.Publisher("Dead_reckoning", Dead_reckoning, queue_size=10)
   ...
   def updatePoseUsingWheel(self, enc_left_tot, enc_right_tot):
      ...
      motion_model = Dead_reckoning() # create msg
      motion_model.d_l = b_l
      motion_model.d_r = b_r
      self.motion_model_pub.publish(motion_model)
